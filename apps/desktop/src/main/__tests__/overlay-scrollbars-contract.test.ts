...
describe('OverlayScrollbars integration contract', () => {
  ...
  it('wraps the vendor library in a viewport-ref preserving React primitive', async () => {
    ...
    assert.match(src, /autoHide:\s*'move'/, 'OverlayScrollArea should use QoderWork-like overlay scrollbars that appear on pointer motion');
  ...
  it('backs COSS ScrollArea with OverlayScrollbars instead of Base UI scrollbars', async () => {
    const cossScrollArea = await repoFile('packages/ui/src/coss/scroll-area.tsx');

    assert.match(cossScrollArea, /OverlayScrollArea/, 'COSS ScrollArea must render the shared OverlayScrollArea primitive');
    assert.doesNotMatch(cossScrollArea, /@base-ui\/react\/scroll-area/, 'COSS ScrollArea must not reintroduce Base UI ScrollArea');
  });
