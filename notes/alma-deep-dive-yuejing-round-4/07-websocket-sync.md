# 07 — Alma WebSocket sync: 9 channels, single port, surgical state push
...
port discovered through `~/.config/alma/api-spec.md` (round-4
...
- **`/ws/browser-relay?token=…`** validates against `Ey.validateToken`
  (`main.js:59250`). The token is generated + persisted in alma
...
- **Everything else: no auth.** Bound to 127.0.0.1; same trust
  ...
  could spoof a renderer connection — alma takes the
...
When alma's REST + WS architecture lands, Maka would gain:
