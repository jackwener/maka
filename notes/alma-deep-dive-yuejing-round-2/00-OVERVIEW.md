...
| 03 | `03-current-maka-oauth-request-shape.md` | Point-in-time diff of Maka's OAuth send path against alma — ranked actionables | **shipped** |
...
| 06 | `06-tool-routing.md` | Pre-loop selector (per user message) + in-loop ToolSearch — alma's two-layer tool routing | **shipped** |
...
Three patterns appear in multiple notes — they're alma's
...
3. **Self-referential agents.** `alma-guide` and `alma-operator`
   (note 07) read/modify the host app's own settings via local
   API endpoints. This is how alma exposes "change my theme",
   "what's my current model?" as natural-language operations
   without touching the renderer.

## Borrowable shortlist for Maka

The single biggest architectural gap between Maka and alma is
